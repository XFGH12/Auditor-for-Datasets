...
def daytime(time,daycycle):
    ...
    tz_data = daycycle['timezone']
    
    if(year_data != None):
        month_day = time.strftime("%m-%d")
        month_day_data = year_data.get(month_day)
    
        if(month_day_data != None):
            sunrise_time = month_day_data["sunrise"]
            sunrise_datetime = year + "-" + month_day +" "+ sunrise_time
            sunrise_datetime= str_to_time(sunrise_datetime, tz_data)
            sunset_time = month_day_data["sunset"]
            sunset_datetime = year + "-" + month_day +" "+ sunset_time
            sunset_datetime= str_to_time(sunset_datetime, tz_data)
                
    if sunrise_datetime < time < sunset_datetime:
        return True
    else:
        return False
