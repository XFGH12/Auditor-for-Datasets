...
def has_instrument_rating(takeoff,student):
    ...
    date_object = utils.str_to_time(student[7])
    #print(date_object)
    #print(type(takeoff))
    
    for item in student[7]:
        if item != None and item !='':
            if date_object < takeoff:
                return True
    return False
...
def has_advanced_endorsement(takeoff,student):
    ...
    date_object = utils.str_to_time(student[8])
    #print(date_object)
    #print(type(takeoff))
    
    for item in student[8]:
        if item != None and item !='':
            if date_object < takeoff:
                return True
    return False
...
def has_multiengine_endorsement(takeoff,student):
    ...
    date_object = utils.str_to_time(student[9])
    #print(date_object)
    #print(type(takeoff))
    
    for item in student[9]:
        if item != None and item !='':
            if date_object < takeoff:
                return True
    return False
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    # Check the first four columns for matching data. (Catagory, Conditions, Area, Time)
    # Search for most advantageous values for Ceiling, Visiblity, Wind, Crosswind:
    #1.Lower values for Ceiling & Visiblity are better.
    #2.Higher values for Wind & Crosswind are better.
    # Returns all 4 values as a float in the same order as they appear in the table.
    #No matching parameters, returns None.
    ...
    for row in minimums[1:]:
        ...
        match1 = False
        if row[0] == 'Student':
            collumn0 = cert >= PILOT_STUDENT
        elif row[0] == 'Certified':
            collumn0 = cert >= PILOT_CERTIFIED
        elif row[0] == '50 Hours':
            collumn0 =  cert >= PILOT_50_HOURS
        elif row[0] == 'Dual':
            collumn0 = instructed
        
        #second column
        collumn1 = row[1] == ('VMC' if vfr else 'IMC')
        
        #third column
        collumn2 = (row[2] == area) or (row[2] == 'Local' and area != 'Cross Country') or (row[2] == 'Any')
        
        #fourth column
        collumn3 = row[3] == ('Day' if daytime else 'Night')

        if collumn0 and collumn1 and collumn2 and collumn3:
