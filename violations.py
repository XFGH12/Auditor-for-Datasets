...
# if visibility is string
    ...
    # else visibility is dictionary

    else:

        # if minimum is a key in dictionary

        if('minimum' in visibility.keys()):

            # if units in FT, convert the visibility->minimum value to miles

            if(visibility['units']=="FT"):

                # FT to SM conversions = divide FT unit by 5280
                result = visibility['minimum']/5280
            ...
            if(visibility['units']=="SM"):
                ...
                result = visibility['minimum']

            # if  minimum is less than visibilty minimum then true

            # as per example test case given
            ...
            # else we compare on basis of prevailing factor

            if(visibility['units']=='FT'):

                result = visibility['prevailing']/5280
                if minimum > result:
                    return True
                else:
                    return False
...
def bad_winds(winds,maxwind,maxcross):
   ...
   # A valid visibility measurement is EITHER the string 'unavailable'
    ...
    # A valid visibility measurement is EITHER the string 'calm'
...
def bad_ceiling(ceiling,minimum):
    ...
    length_ceiling = len(ceiling)
    
    if type(ceiling) is str:
    ...
    height = []
    for i in range(length_ceiling):
        
        if type(ceiling[i]) is dict:

            
            if 'type' not in ceiling[i] and 'height' not in ceiling[i] and 'units' not in ceiling[i]:
                return False
            
            else:
                if ceiling[i]['type'] == 'overcast' or ceiling[i]['type'] == 'broken' or ceiling[i]['type'] == 'indefinite ceiling':
                    if 'height' in ceiling[i]:
                        height.append(ceiling[i]['height'])
                        
                elif ceiling[i]['type'] == 'scattered' or ceiling[i]['type'] == 'a few':
                    if 'height' in ceiling[i]:
                        height.append(500000000)
...
def get_weather_report(takeoff,weather):
    ...
    time = takeoff.isoformat()
 
    if time in weather.keys():
        return weather[str(time)]
        
    for k in weather.keys():
        weather_times = k
        temptime = utils.str_to_time(weather_times,takeoff.tzinfo)
        if temptime < takeoff:
            return weather[k]
...
def get_weather_violation(weather,minimums):
    ...
    if weather == None:
 # If weather is None, then readings not available
        return "Unknown"

    problems = []  # creating a list to append all the problems
 ...
 # If ceiling is bad
    ...
    if len(problems) > 1:  # if there are multiple probless
        return "Weather"
    elif len(problems) == 0:
        return ''
    else:
        return problems[0]  # if there is only one problem
...
def list_weather_violations(directory):
    ...
    violation_table = []
    #Get takeoff time in lessons and convert to datetime object.
    for line in lessons[1:]:
        student_id = line[0]
        takeoff = line[3]
        takeoff_datetime = utils.str_to_time(takeoff[0:18],'America/New_York' )

        
        #Get Boolean Value if Instructor is present during Lesson.
        instructed = False
        instructor = line[2]
        if len(instructor) >= 1:
            instructed = True
        ...
        for student in students:
            if(student[0] == student_id):
                ...
                break
        pilot_creds = pilots.get_certification(takeoff_datetime, student_list)
        ...
        #Get Boolean Value for Flight; True if VFR, else False
        vfr = False
        filled = line[5]
        if filled == 'VFR':
            #if instructed == True:
        ...
        #Get daytime
        daytime_info = utils.daytime(takeoff_datetime,daycycle)
        
        #Get pilot minimums
        pilot_mins = pilots.get_minimums(pilot_creds, area_list, instructed, vfr, daytime_info, minimums)
        #print(f'pilot_mins:{pilot_mins}')
        
        
        #Get weather conditions
        #for data in weather:
        weather_cond = get_weather_report(takeoff_datetime,weather)
        #print(weather_cond)
        
        #Get Violations
        violations = get_weather_violation(weather_cond,pilot_mins)
        #print(violations)
        
        #if(student_id == 'S00874' and line[1] =='426JQ' and line[3]=='2017-04-20T09:00:00-04:00'):
            #print(f'takeoff_datetime {takeoff_datetime} instructor:{instructor} daytime_info={daytime_info} student_id {student_id} certification {pilot_creds} pilot_mins:{pilot_mins} weather_cond:{weather_cond} violations:{violations}')
        
        
        #Return Lessons with Violations
        
        header = line
 
        if len(violations) > 0:
            violation_table.append(header+[violations])
